...
import { ChevronLeft, ChevronRight, Copy, Download, Linkedin, Youtube } from 'lucide-react';
import { Post, Workspace, POST_TYPES, PLATFORMS, POST_STATUSES } from '@/types';
import PostDetailsModal from './PostDetailsModal';
import { format, isSameDay, startOfDay, isToday } from 'date-fns';
import { toast } from '@/hooks/use-toast';
import { useTranslation } from 'react-i18next';

interface GeneralCalendarViewProps {
  posts: Post[];
  workspaces: Workspace[];
}

const GeneralCalendarView: React.FC<GeneralCalendarViewProps> = ({ posts, workspaces }) => {
  ...
  const downloadMedia = async (mediaUrls?: string[]) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${mediaUrls.length} arquivo(s) sendo baixado(s).`,
      });
    } catch (err) {
      toast({
        title: "Erro no download",
        description: "Não foi possível baixar as mídias.",
        variant: "destructive",
      });
    }
  };
              ...
              <PopoverContent className="w-64 p-2" align="start">
                <div className="space-y-1">
                  <div className="text-xs font-medium text-muted-foreground mb-2">
                    {t('calendar.postsOf')} {format(date, 'dd/MM/yyyy')}
                  </div>
  ...
  const renderPostWithWorkspace = (post: Post) => {
    const workspace = workspaceMap[post.workspace_id];
    const workspaceName = workspace?.name || 'Workspace Desconhecido';
    const isRejected = post.status === 'Reprovado';

    return (
      <div
        key={post.id}
        className={`flex items-center justify-between p-3 border rounded-lg hover:bg-muted/20 transition-colors relative ${isRejected ? 'opacity-60' : ''}`}
      >
        {/* Faixa vertical colorida */}
        <div className={`absolute left-0 top-0 bottom-0 w-1 ${getPostTypeColor(post.post_type)} rounded-l-lg`}></div>
        
        <div 
          className="flex items-center space-x-3 flex-1 ml-3 min-w-0 cursor-pointer"
          onClick={() => setSelectedPost(post)}
        >
  ...
  return (
    <div className="space-y-6">
      {/* Top Section - Posts de Hoje em duas colunas */}
      <Card>
        <CardHeader>
        ...
        <CardContent>
          {todayPosts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              {todayPosts.map((post) => renderPostWithWorkspace(post))}
            </div>
          )}
        </CardContent>
      </Card>
          ...
          <div className="calendar-container w-full">
            <Calendar
              ...
              classNames={{
                ...
                row: "flex w-full",
                cell: "text-center text-sm p-0 relative [&:has([aria-selected])]:bg-accent first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md focus-within:relative focus-within:z-20 flex-1 min-h-[80px] border border-border/40",
                day: "w-full h-full p-0 font-normal aria-selected:opacity-100 hover:bg-accent hover:text-accent-foreground rounded-md",
              ...
              components={{
                Day: ({ date }) => {
                  const isCurrentMonth = date.getMonth() === currentCalendarMonth.getMonth() && date.getFullYear() === currentCalendarMonth.getFullYear();
                  return renderDayContent(date, isCurrentMonth);
                }
              }}
            />
          </div>
          ...
          <CardContent>
            {selectedDatePosts.length === 0 ? (
              <p className="text-sm text-muted-foreground">
                {t('calendar.noPostsScheduled')} {format(selectedDate, 'dd/MM/yyyy')}.
              </p>
            ) : (
              <div className="space-y-3">
                {selectedDatePosts.map((post) => renderPostWithWorkspace(post))}
              </div>
            )}
          </CardContent>
        </Card>
