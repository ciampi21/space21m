-- Update cleanup_post_media_assets function to handle thumbnail_urls array
...
BEGIN
  ...
  -- Process legacy thumbnail_url if exists (backward compatibility)
  IF OLD.thumbnail_url IS NOT NULL AND OLD.thumbnail_url != '' THEN
    ...
    IF thumbnail_key IS NOT NULL THEN
      ...
      RAISE LOG 'Marked legacy thumbnail as deleted: % (rows affected: %)', thumbnail_key, deleted_count;
    END IF;
  END IF;
